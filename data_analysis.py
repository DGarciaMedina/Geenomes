...
def plot_bar_chart_for_pathways(matrix,list_of_pathways):
    ...
    y_sorted, list_of_pathways_sorted = zip(*sorted(zip(y, list_of_pathways),reverse = True))
    ...
    plt.xticks(np.arange(len(y_sorted)),list_of_pathways_sorted)

    return np.arange(len(y)),y,list_of_pathways_sorted,y_sorted
    
    
#x,y,x_sorted,y_sorted = plot_bar_chart_for_pathways(matrix,list_of_pathways)

#print(np.array(matrix))
#print(dict_of_genes)

def get_X_for_clustering(cell_line_name):
    d0 = df[cell_line_name+".Control"]
    d3 = df[cell_line_name+".D3"]
    d21 = df[cell_line_name+".D21"]
    
    d0 = np.array([float(i) for i in d0])
    d3 = np.array([float(i) for i in d3])
    d21 = np.array([float(i) for i in d21])
    ...
    for i in range(len(d0)):
        X.append([param1[i],param2[i]])
        
    return X,param1,param2

def perform_clustering(name,plot_number):
    '''
    Takes a list of lists of size 2. E.g.
    
    X = [[x0,y0],[x1,y1],[x2,y2]]
    
    '''
    X,param1,param2 = get_X_for_clustering(name)
...
#    return labels,param1,param2
    
    plt.figure(plot_number)
    plt.plot(param1,param2,"o")

#labels,param1,param2 = perform_clustering()

#print("colouring started")
#plt.figure(3)
...
#    else:
#        colour = "green"
#    plt.plot(param1[i],param2[i],"o",color=colour)

#plt.plot(param1,param2,"o")
#plt.figure()

#cell_line_list = ["M233","M262"]
cell_line_list = []

for i,column_head in enumerate(list(df.columns.values)):
    if i % 3 == 0 and column_head != "symbol":
        print(column_head)
        cell_line_list.append(column_head.split(".")[0])

for i,cell_line in enumerate(cell_line_list):
    perform_clustering(cell_line,i)
